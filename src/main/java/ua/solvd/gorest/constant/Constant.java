package ua.solvd.gorest.constant;

public class Constant {
    public static final String AUTH_URL = "https://gorest.co.in/public/v2";
    public static final String HEADER_AUTHORIZATION = "Authorization";
    public static final String HEADER_AUTHORIZATION_BEARER = "Bearer ";
    public static final String DEFAULT_USERNAME = "John Doe";
    public static final String INVALID_USERNAME = "Invalid Email User";
    public static final String MALE_GENDER = "male";
    public static final String FEMALE_GENDER = "female";
    public static final String STATUS_ACTIVE = "active";
    public static final String DEFAULT_EMAIL = "@example.com";
    public static final String INVALID_EMAIL = "Invalid Email Address";
}